...
export function validateRecipeId(recipeId: string): ValidationError | null {
  ...
  if (!/^[a-z0-9_]+:[a-z0-9_/]+$/.test(recipeId)) {
    return {
      field: "recipeId",
      message:
        'Recipe ID must be in format "namespace:path" (lowercase, numbers, underscores only)',
      severity: "error",
    };
  }
...
export function validateFluidInput(
  fluid: any,
  fieldName: string = "fluid"
...
export function validateProcessingRecipe(
  recipe: any,
  config: {
