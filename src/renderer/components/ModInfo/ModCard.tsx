...
export function ModCard({ mod }: ModCardProps) {
  ...
  return (
    <div className={`space-y-${compactView ? "4" : "6"} animate-fadeIn`}>
      {/* Mod Header */}
      <div
        className={`border border-border rounded-xl ${padding} bg-gradient-to-br from-card to-card/50 shadow-lg backdrop-blur-sm`}
      >
        <div className="flex items-start gap-4">
          {mod.icon ? (
            ...
            <div className="w-20 h-20 rounded-xl bg-gradient-to-br from-primary/20 to-purple-500/20 flex items-center justify-center text-primary text-3xl font-bold shadow-lg ring-2 ring-primary/20">
              {mod.name.substring(0, 2).toUpperCase()}
            </div>
          )}

          <div className="flex-1">
            <h2 className="text-2xl font-bold mb-1 bg-gradient-to-r from-foreground to-foreground/80 bg-clip-text">
              {mod.name}
            </h2>
            <p className="text-muted-foreground flex items-center gap-2">
              <span className="px-2 py-0.5 bg-primary/10 text-primary rounded font-medium text-sm">
                v{mod.version}
              </span>
        ...
        {mod.description && (
          <p className="mt-4 text-muted-foreground leading-relaxed border-l-2 border-primary/30 pl-4">
            {mod.description}
          </p>
        )}
        ...
        <div className="flex flex-wrap gap-2 mt-4">
          {mod.homepage && (
            <a
              ...
              className="px-3 py-2 text-sm bg-secondary hover:bg-secondary/80 rounded-lg transition-all hover:scale-105 flex items-center gap-2 shadow-sm font-medium"
            >
              <svg
                className="w-4 h-4"
                fill="none"
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="px-3 py-2 text-sm bg-orange-500/10 text-orange-500 hover:bg-orange-500/20 rounded-lg transition-all hover:scale-105 flex items-center gap-2 shadow-sm font-medium"
          >
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="px-3 py-2 text-sm bg-green-500/10 text-green-500 hover:bg-green-500/20 rounded-lg transition-all hover:scale-105 flex items-center gap-2 shadow-sm font-medium"
          >
      ...
      {/* Additional Info */}
      <div className="border border-border rounded-xl p-6 bg-card shadow-md">
        ...
        <dl className="space-y-3 text-sm">
          <div className="flex justify-between items-center p-2 rounded-lg hover:bg-accent/30 transition-colors">
            <dt className="text-muted-foreground font-medium">Mod ID:</dt>
            <dd className="font-mono text-xs bg-secondary px-2 py-1 rounded">
              {mod.modId}
            </dd>
          </div>
          <div className="flex justify-between items-center p-2 rounded-lg hover:bg-accent/30 transition-colors">
            <dt className="text-muted-foreground font-medium">Version:</dt>
            <dd className="font-semibold">{mod.version}</dd>
          </div>
          <div className="flex justify-between items-center p-2 rounded-lg hover:bg-accent/30 transition-colors">
            <dt className="text-muted-foreground font-medium">Loader:</dt>
          ...
          {mod.license && (
            <div className="flex justify-between items-center p-2 rounded-lg hover:bg-accent/30 transition-colors">
              ...
              <dd className="font-semibold">{mod.license}</dd>
            </div>
          )}
        </dl>
      </div>
