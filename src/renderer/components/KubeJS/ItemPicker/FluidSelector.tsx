...
export const FluidSelector: React.FC<FluidSelectorProps> = ({ onSelect, onClose }) => {
  ...
  const filteredFluids = useMemo(() => {
    ...
    return fluids.filter(
      (fluid) => fluid.id.toLowerCase().includes(query) || fluid.name.toLowerCase().includes(query)
    );
  }, [searchQuery, fluids]);
  ...
  return (
    <>
      <div className="p-4 border-b border-primary/20">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 bg-secondary border border-primary/20 rounded text-foreground placeholder-muted-foreground focus:outline-none focus:border-primary"
            autoFocus
          />
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4">
        {isLoading ? (
          <div className="text-center text-muted-foreground py-8">Loading fluids...</div>
        ) : filteredFluids.length === 0 ? (
          <div className="text-center text-muted-foreground py-8">No fluids found</div>
        ) : (
          <div className="grid grid-cols-1 gap-2">
            {filteredFluids.map((fluid) => (
              <button
                ...
                className="flex items-center gap-3 p-3 bg-secondary hover:bg-secondary/80 border border-primary/20 rounded transition-colors text-left"
              >
                <div className="w-10 h-10 bg-secondary border border-primary/20 rounded flex items-center justify-center">
                  {fluid.texture ? (
                    <img
                      src={
                        fluid.texture.startsWith("data:")
                          ? fluid.texture
                          : `data:image/png;base64,${fluid.texture}`
                      }
                      alt={fluid.name}
