...
export const TagSelector: React.FC<TagSelectorProps> = ({ onSelect, onClose, type = "item" }) => {
  ...
  const filteredTags = useMemo(() => {
    ...
    return COMMON_TAGS.filter(
      (tag) => tag.id.toLowerCase().includes(query) || tag.name.toLowerCase().includes(query)
    );
  }, [searchQuery]);
        ...
        <div className="space-y-3">
          ...
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 px-4 py-2 bg-secondary border border-primary/20 rounded text-foreground placeholder-muted-foreground focus:outline-none focus:border-primary"
            />
            <button
