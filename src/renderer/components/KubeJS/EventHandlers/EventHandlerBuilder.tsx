...
const EVENT_TYPES = [
  ...
  {
    id: "player.logged_out",
    name: "Player Logout",
    description: "When a player leaves the server",
  },
  ...
  return (
    <div className="bg-card rounded-lg p-6 border border-primary/20">
      ...
      <div className="mb-6">
        ...
        {conditions.length === 0 ? (
          <p className="text-sm text-muted-foreground italic">
            No conditions. Event will trigger for all cases.
          </p>
        ) : (
          <div className="space-y-2">
            {conditions.map((condition, index) => (
              <div
                key={index}
                className="bg-background p-3 rounded border border-primary/20 flex gap-2"
              >
                <select
                  value={condition.type}
                  onChange={(e) => handleUpdateCondition(index, "type", e.target.value)}
                  className="flex-1 bg-card text-foreground px-3 py-2 rounded border border-primary/20 focus:border-primary focus:outline-none"
                >
                ...
                {condition.type === "block" || condition.type === "item" ? (
                  <UnifiedSelector
                    ...
                    placeholder={
                      CONDITION_TYPES.find((ct) => ct.id === condition.type)?.valueLabel || "Value"
                    }
                  />
                ) : (
                  <input
                ...
                {action.type !== "cancel_event" &&
                  (action.type === "give_item" || action.type === "remove_item" ? (
                    <UnifiedSelector
                      ...
                      type="item"
                      placeholder={
                        ACTION_TYPES.find((at) => at.id === action.type)?.valueLabel || "Value"
                      }
                    />
      ...
      {eventType && actions.length > 0 && (
        <div className="mb-6 bg-muted/50 p-4 rounded border border-primary/20">
