  ...
  return (
    <div className="space-y-2 mt-4">
      {validation.errors.length > 0 && (
        <div className="bg-destructive/10 border border-red-500/30 rounded-lg p-3">
          ...
          <ul className="space-y-1 text-sm">
            {validation.errors.map((error, index) => (
              <li key={index} className="text-muted-foreground">
                <span className="font-medium text-destructive">{error.field}:</span> {error.message}
              </li>
            ))}
          </ul>
        </div>
        ...
        <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-lg p-3">
          ...
          <ul className="space-y-1 text-sm">
            {validation.warnings.map((warning, index) => (
              <li key={index} className="text-muted-foreground">
                <span className="font-medium text-yellow-400">{warning.field}:</span>{" "}
                {warning.message}
              </li>
            ))}
          </ul>
