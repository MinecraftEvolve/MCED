...
const CreateCompactingEditor: React.FC<CreateCompactingEditorProps> = ({ instancePath, onSave, initialRecipe }) => {
  const [inputs, setInputs] = useState<(string | null)[]>(initialRecipe?.inputs || [null, null, null, null]);
...
${inputs.filter(i => i).map(input => `  '${input}'`).join(',\n')}
