...
export const FluidSlot: React.FC<FluidSlotProps> = ({
  ...
  return (
    <div 
      ...
      onClick={onClick}
      title={normalizedFluid ? `${normalizedFluid.name}\n${normalizedFluid.id}` : 'Click to select fluid'}
    >
      {normalizedFluid ? (
        <>
          <div className={`absolute inset-0 flex items-center justify-center ${padding}`}>
            {normalizedFluid.texture ? (
              <img
                src={normalizedFluid.texture.startsWith('data:') ? normalizedFluid.texture : `data:image/png;base64,${normalizedFluid.texture}`}
          ...
          {allowAmount && onAmountChange && (
            <input
              ...
              className="absolute bottom-1 right-1 w-16 h-6 bg-background text-foreground text-xs font-bold px-1 rounded shadow-lg border border-border text-center"
            />
          )}
        </>
      ) : (
