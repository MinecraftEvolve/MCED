    ...
    >
      {normalizedItem ? (
        <>
          <div className={`absolute inset-0 flex items-center justify-center ${padding}`}>
            {normalizedItem.texture ? (
              <img
                src={
                  normalizedItem.texture.startsWith("data:")
                    ? normalizedItem.texture
                    : `data:image/png;base64,${normalizedItem.texture}`
                }
          ...
          {allowCount && onCountChange && (
            <input
              ...
              className="absolute bottom-1 right-1 w-10 h-6 bg-background text-foreground text-xs font-bold px-1 rounded shadow-lg border border-primary/20 text-center"
            />
          )}
          ...
          <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 hidden group-hover:block z-10 pointer-events-none">
            <div className="bg-popover text-popover-foreground text-xs rounded px-2 py-1 whitespace-nowrap shadow-lg border border-primary/20">
              ...
              {displayCount && displayCount > 1 && (
                <div className="text-muted-foreground">Count: {displayCount}</div>
              )}
              {normalizedItem.chance !== undefined && normalizedItem.chance < 1 && (
                <div className="text-yellow-400">
                  Chance: {Math.round(normalizedItem.chance * 100)}%
                </div>
              )}
            </div>
          </div>
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <Plus
            className={`${plusSize} text-muted-foreground/30 group-hover:text-primary/50 transition-colors`}
          />
        </div>
      )}
    </div>
