...
export class JarLoaderService {
  ...
  async detectLauncher(instancePath: string): Promise<LauncherType> {
    ...
    const hasProfileJson = await fs.stat(profileJsonPath).then(() => true).catch(() => false);
    const hasModrinthDb = await fs.stat(modrinthDbPath).then(() => true).catch(() => false);
  ...
  private async _loadJarInternal(jarPath: string, forceModId?: string): Promise<JarData> {
    console.log(`Loading JAR: ${path.basename(jarPath)}`);
    ...
    console.log(`  Loaded from ${path.basename(jarPath)}: ${items.length} items, ${fluids.length} fluids, ${tags.size} tags`);

    return { modId, items, fluids, tags };
  }
  ...
  async loadAllJars(modsFolder: string): Promise<JarData[]> {
    ...
    this.loadAllPromise = (async () => {
      ...
      console.log(`Loading ${jarFiles.length} JAR files in parallel...`);
      
      const results = await Promise.all(
        jarFiles.map(jar => this.loadJar(path.join(modsFolder, jar)))
      );
    ...
    try {
      ...
      if (hasProfileJson) {
        ...
        try {
          ...
          console.log(`Loaded profile from profile.json: MC ${mcVersion}, ${loaderType} ${loaderVersion}`);
        } catch (error) {
        ...
        if (profile && profile.game_version) {
          ...
          console.log(`Loaded profile from Modrinth database: MC ${mcVersion}, ${loaderType} ${loaderVersion}`);
        } else {
        ...
        if (profile) {
          ...
          console.log(`Loaded profile from CurseForge: MC ${mcVersion}, ${loaderType} ${loaderVersion}`);
        } else {
        ...
        if (profile) {
          ...
          loaderType = profile.loader.toLowerCase();
          loaderVersion = profile.loaderVersion;
          console.log(`Loaded profile from Packwiz: MC ${mcVersion}, ${loaderType} ${loaderVersion}`);
        } else {
      ...
      const curseForgeBaseMeta = path.join(path.dirname(path.dirname(instancePath)), 'meta', 'versions');
      ...
      if (loaderType === 'forge' || loaderType === 'neoforge') {
        ...
        const curseForgeInstallLoaderPath = path.join(curseForgeBaseInstall, loaderFolderName, loaderJarName);
        ...
        const curseForgeMetaLoaderPath = path.join(curseForgeBaseMeta, loaderVersionStr, `${loaderVersionStr}.jar`);
      ...
      const curseForgeMetaVanillaPath = path.join(curseForgeBaseMeta, mcVersion, `${mcVersion}.jar`);
      ...
      const curseForgeInstallVanillaPath = path.join(curseForgeBaseInstall, mcVersion, `${mcVersion}.jar`);
