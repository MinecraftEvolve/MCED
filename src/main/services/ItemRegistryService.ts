...
export class ItemRegistryService {
  ...
  private async loadVanillaItemsFromForge(
    modsFolder: string
  ): Promise<{ items: ItemInfo[]; blocks: ItemInfo[] } | null> {
    try {
  ...
  private async findForgeJar(modsFolder: string): Promise<string | null> {
    try {
      ...
      if (userProfile) {
        const curseforgeVersions = path.join(
          userProfile,
          "curseforge",
          "minecraft",
          "Install",
  ...
  private async extractTexturesFromJar(
    ...
    try {
      ...
      // Extract blocks from JarLoader data
      for (const block of jarData.blocks) {
        blocks.push({
          id: block.id,
          modId: jarData.modId,
          name: block.name,
          texture: `data:image/png;base64,${block.texture}`,
          type: "block",
        });
      }
    } catch (error) {
  ...
  async getAllItems(): Promise<ItemInfo[]> {
    ...
    for (const item of combined) {
      ...
      if (!existing) {
        ...
        const existingScore =
          (existing.texture ? 1 : 0) + (existing.name ? 1 : 0) + (existing.modId ? 1 : 0);
  ...
  private async loadTagFromModJars(namespace: string, tagPath: string): Promise<ItemInfo[]> {
    try {
      ...
      const instancePath = this.cacheDir
        .replace(path.join(".mced", "item-cache"), "")
        .replace(/[\/\\]$/, "");
