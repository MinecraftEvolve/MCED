...
export class RecipeService {
  ...
  async parseRecipeFile(filePath: string): Promise<ParsedRecipe[]> {
    ...
    try {
      ...
      const recipeStarts = /event\.(recipes\.[\w.]+|shaped|shapeless|smelting|blasting|smoking|campfireCooking|stonecutting|smithing)\s*\(/g;
      ...
      while ((match = recipeStarts.exec(content)) !== null) {
        ...
        const validChainMethods = ['heated', 'superheated', 'processingTime', 'id', 'withChance', 'keepHeldItem'];
  ...
  private parseRecipeCall(methodName: string, rawParams: string, fullRecipe: string): ParsedRecipe {
    ...
    if (type === 'create:crushing') {
      ...
      if (outputsArrayMatch) {
        ...
        const itemMatches = outputsContent.matchAll(/["']([^"']+)["']|Item\.of\s*\(\s*["']([^"']+)["']\s*\)(?:\.withChance\s*\(\s*([\d.]+)\s*\))?/g);
        for (const match of itemMatches) {
  ...
  async searchRecipes(query: string): Promise<any[]> {
    ...
    return allRecipes.filter(recipe => {
      const matchesIngredient = recipe.ingredients.some((ing: any) => 
...
${keyEntries}
    ...
    // Create: Crushing
    if (type === 'create:crushing') {
      const outputs = recipe.outputs.map((out: any) => {
