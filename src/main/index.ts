      ...
      instance: {
        ...
        modpack: instance.modpack
          ? {
              ...
              author: instance.modpack.author ? String(instance.modpack.author) : undefined,
        ...
        defaultConfigsFolder: instance.defaultConfigsFolder
          ? String(instance.defaultConfigsFolder)
          : undefined,
        serverConfigFolder: instance.serverConfigFolder
          ? String(instance.serverConfigFolder)
          : undefined,
    ...
    for (const worldName of saves) {
      ...
      for (const fileName of configFiles) {
        ...
        try {
          ...
          errors.push(
            `Failed to migrate ${fileName}: ${error instanceof Error ? error.message : String(error)}`
          );
        }
      }
    }
...
ipcMain.handle(
  "fs:readdirRecursive",
  async (_event, dirPath: string, options?: { extensions?: string[]; maxDepth?: number }) => {
...
ipcMain.handle("fs:joinPath", async (_event, ...paths: string[]) => {
  try {
    ...
    throw new Error(
      `Failed to join paths: ${error instanceof Error ? error.message : String(error)}`
    );
  }
});
    ...
    const plainInstance = {
      ...
      modpack: instance.modpack
        ? {
            ...
            version: String(instance.modpack.version || ""),
            author: instance.modpack.author ? String(instance.modpack.author) : undefined,
            projectId: instance.modpack.projectId ? String(instance.modpack.projectId) : undefined,
          }
        : undefined,
      modsFolder: String(instance.modsFolder || ""),
      configFolder: String(instance.configFolder || ""),
  ...
  try {
    ...
    const serializedMods = mods.map((mod) => {
      ...
      const safeMod = {
        ...
        sources: mod.sources ? String(mod.sources) : undefined,
        license: mod.license ? String(mod.license) : undefined,
        icon: mod.icon && typeof mod.icon === "string" ? String(mod.icon) : undefined,
        jarPath: String(mod.jarPath || ""),
        loader: String(mod.loader || "forge"),
        configFiles: [],
        isFavorite: false,
...
ipcMain.handle("open-in-editor", async (_event, filePath: string, editorCommand?: string) => {
  try {
    if (editorCommand) {
      ...
      const editors = ["code", "code-insiders", "cursor", "subl", "atom", "notepad++"];
      let opened = false;
...
ipcMain.handle(
  "discord:set-mod",
  async (_event, modName: string, modCount: number, configFileName?: string) => {
    try {
...
ipcMain.handle(
  "kubejs:createScript",
  async (_event, instancePath: string, relativePath: string, content: string) => {
    try {
      const kubeJSService = new KubeJSService(instancePath);
...
ipcMain.handle(
  ...
  async (
    ...
    try {
      const kubeJSService = new KubeJSService(instancePath);
      const result = await kubeJSService.organizeScripts(instancePath, options);
...
ipcMain.handle(
  ...
  async (_event, instancePath: string, modsFolder: string) => {
    try {
      const itemRegistry = new ItemRegistryService(instancePath);
...
ipcMain.handle(
  "fluidRegistry:searchFluids",
  async (_event, instancePath: string, query: string) => {
    try {
      const fluidRegistry = new FluidRegistryService(instancePath);
      await fluidRegistry.loadCache();
...
ipcMain.handle(
  "fluidRegistry:getFluidById",
  async (_event, instancePath: string, fluidId: string) => {
    try {
      const fluidRegistry = new FluidRegistryService(instancePath);
      await fluidRegistry.loadCache();
...
ipcMain.handle(
  "recipe:create",
  async (_event, instancePath: string, scriptPath: string, recipe: string) => {
    try {
      const recipeService = new RecipeService(instancePath);
...
ipcMain.handle(
  "recipe:delete",
  async (_event, instancePath: string, scriptPath: string, recipeId: string) => {
    try {
      const recipeService = new RecipeService(instancePath);
...
ipcMain.handle(
  ...
  async (
    ...
    try {
      ...
      const conditionsCode = eventHandler.conditions
        .map((c) => {
          switch (c.type) {
            case "block":
              return `event.block.id === '${c.value}'`;
            case "item":
              return `event.item.id === '${c.value}'`;
            case "entity":
              return `event.entity.type === '${c.value}'`;
            case "player":
              return `event.player.username === '${c.value}'`;
            case "dimension":
              return `event.level.dimension === '${c.value}'`;
            default:
              return "";
          }
        })
      ...
      const actionsCode = eventHandler.actions
        .map((a) => {
          switch (a.type) {
            case "give_item":
              return `  event.player.give('${a.value}'${a.count ? `, ${a.count}` : ""});`;
            case "remove_item":
              return `  event.player.inventory.clear('${a.value}');`;
            case "send_message":
              return `  event.player.tell('${a.value}');`;
            case "spawn_entity":
              return `  event.level.spawnEntity('${a.value}', event.player.blockPosition());`;
            case "set_block":
              return `  event.level.setBlock(event.blockPosition, '${a.value}', 3);`;
            case "run_command":
              return `  event.server.runCommandSilent('${a.value}');`;
            case "potion_effect":
              return `  event.player.potionEffects.add('${a.value}', 200, 0);`;
            default:
              return "";
          }
        })
